...
public class game {
		...
		// N games are simulated
					...
					for (int counter = 0; counter != N; ++counter) {
						...
						while (i < 450 || t1.stats.score() == t2.stats.score) {
							...
							if (i % 2 == 0) // team 1 shooting (5 players)
							{
								if (shooter < 31) {
							...
							else // team 2 shooting (6 players)
							{
								if (shooter < 21) {
								twoPoint_fix(t2.p1, t1.p1, t2, shot);
								threePoint_fix(t2.p1, t1.p1, t2, shot);
								i++;
								}
								if (shooter > 20 && shooter < 41) {
									twoPoint_fix(t2.p2, t1.p2, t2, shot);
									threePoint_fix(t2.p2, t1.p2, t2, shot);
									i++;
								}
								if (shooter > 40 && shooter < 59) {
									twoPoint_fix(t2.p3, t1.p3, t2, shot);
									threePoint_fix(t2.p3, t1.p3, t2, shot);
									i++;
								}
								if (shooter > 58 && shooter < 76) {
									twoPoint_fix(t2.p4, t1.p4, t2, shot);
									threePoint_fix(t2.p4, t1.p4, t2, shot);
									i++;
								}
								if (shooter > 75 && shooter < 89) {
									twoPoint_fix(t2.p5, t1.p5, t2, shot);
									threePoint_fix(t2.p5, t1.p5, t2, shot);
									i++;
								}
								if (shooter > 89) {
									twoPoint_fix(t2.p6, t1.p6, t2, shot);
									threePoint_fix(t2.p6, t1.p6, t2, shot);
									i++;
								}
							}
						}
	...
	public static void playerStats(Team team) {
		...
		System.out.println(team.p5.getName() + ": " + team.p5.stats.score() + " points (" + team.p5.stats.threes
				+ " three pointers)");
		System.out.println(team.p6.getName() + ": " + team.p6.stats.score() + " points (" + team.p6.stats.threes
				+ " three pointers)");
	}
	...
	// odds of shooting and making a 2 point shot for 2nd team
	public static void twoPoint_fix(Player offense, Player defense, Team team, int shot) {
		int chance = randInt(1, 100);
		int defence_quality = randInt(1, 100);
		if (shot < 85 && chance < offense.twoPt() && defence_quality > defense.defense()) {
			offense.stats.addScore(1.67);
			team.stats.addScore(1.67);
		}
	}

	// odds of shooting and making a 3 point shot for 2nd team
	public static void threePoint_fix(Player offense, Player defense, Team team, int shot) {
		int chance = randInt(1, 100);
		int defence_quality = randInt(1, 100);
		if (shot >= 85 && chance < offense.threePt() && defence_quality > defense.defense()) {
			offense.stats.addScore(2.5);
			offense.stats.addThree(1);
			team.stats.addScore(2.5);
		}
	}
